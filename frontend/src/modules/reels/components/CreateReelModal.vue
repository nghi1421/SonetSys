    ...
    >
      <div class="w-full max-w-md rounded-hud border border-cyber-border bg-cyber-glass p-5 backdrop-blur-md" @click.stop>
        ...
        <div v-if="mediaPreviewUrl" class="relative mt-3 w-fit">
          <video :src="mediaPreviewUrl" controls class="max-h-72 rounded-hud border border-cyber-border" />
          <button
            type="button"
        ...
        >
          <Video class="h-6 w-6" />
          <span class="font-mono text-[10px] uppercase tracking-widest">{{ t('reels.createModal.addVideo') }}</span>
        </button>
